...
dl_bool _confirm_properties(dl_collection *c, const char *type_name) {
  ...
  dl_iterator last = dl_make_invalid_iterator(c);
...
dl_bool test_linked_list_add() {
  ...
  current = 0;
  while (node != NULL) {
    value = *(dl_natural *)DL_LINKED_LIST_DATA(node);
...
dl_bool test_linked_list_copy() {
  ...
  while (node_a != NULL && node_b != NULL) {
    value_a = *(dl_natural *)DL_LINKED_LIST_DATA(node_a);
    value_b = *(dl_natural *)DL_LINKED_LIST_DATA(node_b);
...
dl_bool test_linked_list_copy_array() {
  ...
  for (idx = 0;
    idx < num && node != NULL;
    ++idx, node = node->next) {
    value = *(dl_integer *)DL_LINKED_LIST_DATA(node);
